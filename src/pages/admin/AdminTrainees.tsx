...
const AdminTrainees: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...traineeData.map(t => [t.id, `"${t.name}"`, t.startDate, `"${t.office}"`, t.status].join(','))
    ].join('\n');
...
.status-icon svg { width: 10px; height: 10px; fill: none; stroke: #fff; stroke-width: 3; stroke-linecap: round; stroke-linejoin: round; }
.status-active { background: linear-gradient(135deg, #d1fae5 0%, #ecfdf5 100%); color: #065f46; border: 1.5px solid #6ee7b7; box-shadow: 0 2px 6px rgba(6,95,70,.12), inset 0 1px 0 rgba(255,255,255,.6); }
.status-active .status-icon { background: linear-gradient(135deg, #059669, #10b981); box-shadow: 0 1px 4px rgba(5,150,105,.35); }
.status-inactive { background: linear-gradient(135deg, #fef3c7 0%, #fffbeb 100%); color: #78350f; border: 1.5px solid #fcd34d; box-shadow: 0 2px 6px rgba(120,53,15,.1), inset 0 1px 0 rgba(255,255,255,.6); }
.status-inactive .status-icon { background: linear-gradient(135deg, #d97706, #f59e0b); box-shadow: 0 1px 4px rgba(217,119,6,.35); }
              ...
              <div className="trainee-row">
                ...
                <div>
                  <span className="status-badge status-active">
                    <span className="status-icon"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg></span>
                    Active
                  </span>
                </div>

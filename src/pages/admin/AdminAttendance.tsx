  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      ...ATTENDANCE_DATA.map(r => [
        r.id, `"${r.name}"`, r.date, r.status, r.timeIn, r.timeOut,
        r.hoursRendered || '', `"${r.department || ''}"`, `"${r.supervisor || ''}"`,
        `"${(r.remarks || '').replace(/"/g, '""')}"`
      ].join(','))
    ].join('\n');
...
.btn-ghost-sm { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; background: var(--bg); color: var(--ink-2); border: 1px solid var(--rule); border-radius: var(--r-md); font-size: .8rem; font-weight: 500; cursor: pointer; transition: all .2s var(--ease); font-family: 'DM Sans', sans-serif; }
.btn-ghost-sm:hover { background: var(--brand-soft); border-color: var(--brand-glow); color: var(--brand); }
.btn-ghost-sm svg { width: 14px; height: 14px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
...
.search-wrap svg {
  ...
  width: 15px; height: 15px; stroke: var(--ink-3); pointer-events: none; fill: none;
...
/* Each filter group: label + control side-by-side */
.filter-group {
  display: flex;
  align-items: center;
  gap: 8px;
...
.attendance-row { display: grid; grid-template-columns: 1fr 160px 200px 100px; align-items: center; padding: 16px 24px; border-bottom: 1px solid var(--rule); transition: background .15s var(--ease); animation: rowIn .3s var(--ease) both; }
...
.trainee-avatar { width: 42px; height: 42px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-family: 'Syne', sans-serif; font-size: .85rem; font-weight: 700; color: #fff; box-shadow: 0 2px 8px rgba(95,0,118,.25); }
...
.status-badge { display: inline-flex; align-items: center; gap: 7px; padding: 5px 12px 5px 6px; border-radius: var(--r-full); font-size: .775rem; font-weight: 700; letter-spacing: .02em; white-space: nowrap; width: fit-content; }
...
.status-present { background: linear-gradient(135deg,#d1fae5,#ecfdf5); color: #065f46; border: 1.5px solid #6ee7b7; box-shadow: 0 2px 6px rgba(6,95,70,.12), inset 0 1px 0 rgba(255,255,255,.6); }
.status-present .status-icon { background: linear-gradient(135deg,#059669,#10b981); box-shadow: 0 1px 4px rgba(5,150,105,.35); }
.status-late { background: linear-gradient(135deg,#fef3c7,#fffbeb); color: #78350f; border: 1.5px solid #fcd34d; box-shadow: 0 2px 6px rgba(120,53,15,.1), inset 0 1px 0 rgba(255,255,255,.6); }
.status-late .status-icon { background: linear-gradient(135deg,#d97706,#f59e0b); box-shadow: 0 1px 4px rgba(217,119,6,.35); }
.status-absent { background: linear-gradient(135deg,#fee2e2,#fff5f5); color: #991b1b; border: 1.5px solid #fca5a5; box-shadow: 0 2px 6px rgba(153,27,27,.1), inset 0 1px 0 rgba(255,255,255,.6); }
.status-absent .status-icon { background: linear-gradient(135deg,#dc2626,#ef4444); box-shadow: 0 1px 4px rgba(220,38,38,.35); }

.time-range { display: flex; align-items: center; gap: 6px; font-size: .8125rem; color: var(--ink-2); font-weight: 500; }
.time-range svg { width: 13px; height: 13px; stroke: var(--ink-3); fill: none; flex-shrink: 0; }
...
@media (max-width: 840px) {
  ...
  .filters-bar { height: auto; flex-wrap: wrap; gap: 0; padding: 12px; border-radius: var(--r-lg); }
  .search-wrap { width: 100%; height: 40px; border-radius: var(--r-md); border: 1.5px solid var(--rule); margin-bottom: 10px; }
  .filter-sep { display: none; }
  .filter-group { padding: 0; width: 50%; }
}
                ...
                <div className="attendance-row" key={record.id}>
                  ...
                  <div>
                    <span className={`status-badge status-${record.status}`}>
                      <span className="status-icon">{STATUS_ICON[record.status]}</span>
